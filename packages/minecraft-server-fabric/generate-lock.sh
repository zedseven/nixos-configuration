...
| jq -r '
  ...
  |"\($name)|\($version)|\($name)-\($version).zip|\($url)\($dir|sub("\\.";"/";"g"))/\($name)/\($version)/\($name)-\($version).jar"
  )' \
| {
    ...
    echo "  libraries = ["
    while IFS="|" read name version file url; do
        hash=$(nix-prefetch-url $url);
        echo "    { name = \"$name\"; version = \"$version\"; fileName = \"$file\"; url = \"$url\"; sha256 = \"$hash\"; }"
    done
    echo "  ];"
    echo "}"
}
